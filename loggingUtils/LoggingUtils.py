...
def setup_logger(output_file=None, logging_config=None):
    ...
    if logging_config is not None:
        ...
        with open(file=os.path.join(os.path.dirname(__file__), 'logging.yaml'), mode='r', encoding="utf-8") as fh:
            logging_config = yaml.safe_load(fh)
        if output_file is not None:
            logging_config['handlers']['info_file_handler']['filename'] = output_file
        logging.config.dictConfig(logging_config)
